...
const STRONG_PATTERNS = {
    P2_S: [0, 1], P3_S1: [0, 1, 2], P3_S2: [0, 0, 0]
};
...
function generateHand() {
    ...
    while (shanten !== 1) {
        hand = [];
        const allCombos = [
            [8, 3, 3], [7, 4, 3], [6, 5, 3], [6, 4, 4], [5, 5, 4],
            [11, 3, 0], [10, 4, 0], [9, 5, 0], [8, 6, 0], [7, 7, 0], [14, 0, 0]
        ];
    ...
    for (let s of SUITS) {
        for (let n = 1; n <= 9; n++) {
            ...
            if (getShanten(tempHand) < currentShanten) {
                ...
                const remaining = 4 - alreadyHave;
                if (remaining > 0) {
                    tiles.push({ num: n, suit: s, count: remaining });
...
function discardTile(index) {
    ...
    if (userData.nextShanten > baseShanten) {
        ...
        let statusText = isBest ? `<span style="color: #44ff44;">★ Excellent!</span>` : `<span style="color: #f1c40f;">▲ Good</span>`;
        statusText += ` 打:<img src="img/${userDiscard.num}${userDiscard.suit}.png" style="height:35px; vertical-align:middle; margin:0 5px;"> `;
        statusText += `(受入:${userData.ukeire.totalCount}枚) 有効牌: `;
        statusText += userData.ukeire.tiles.map(t => `<img src="img/${t.num}${t.suit}.png" style="height:28px; vertical-align:middle;">`).join('');
    ...
    analysisData.forEach(d => {
        const isBack = d.nextShanten > baseShanten;
        const ukeireHtml = d.ukeire ? d.ukeire.tiles.map(t => `<img src="img/${t.num}${t.suit}.png" style="height:22px;">`).join('') : "-";
        const isBest = d.ukeire && d.ukeire.totalCount === maxUkeire;
        tableHtml += `
            <tr style="${isBest ? 'background: rgba(68,255,68,0.2); font-weight:bold;' : ''}">
                <td><img src="img/${d.tile.num}${d.tile.suit}.png" style="height:40px;"></td>
                <td>${isBack ? '<span style="color:red">向聴戻り</span>' : '維持'}</td>
                <td>${d.ukeire ? d.ukeire.totalCount + '枚' : "-"}</td>
                <td><div class="ukeire-icons">${ukeireHtml}</div></td>
            </tr>`;
    });
    tableHtml += `</table>`;
    document.getElementById('analysis-area').innerHTML = tableHtml;
